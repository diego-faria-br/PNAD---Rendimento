...
*SEG.
VARIABLE LEVEL SEG  (NOMINAL).
VALUE LABELS  SEG
     0  'Potenciais Segurados Rurais'
     1  'Agricultores Familiares'
     2  'Segurados Rurais'
     3  'Servidores Públicos e Militares'
     4  'Segurados Urbanos'
     5  'Não Segurados Urbanos'
     6  'Não Segurados Rurais'
     9  'Sem Aplicação'  .
EXECUTE.
...
IF ((SEG >= 0 & SEG <= 6) & (Segurado = 0)) SFB = 9 .
EXECUTE .

*Define Variable Properties.
*SFB.
VARIABLE LEVEL SFB  (NOMINAL).
VALUE LABELS  SFB
     0  'Potenciais Segurados Rurais'
     1  'Agricultores Familiares'
     2  'Segurados Rurais'
...
IF  ((V4718 >= 0 & V4718 <= 724)) GRUPO_REND=1.
IF  ((V4718 > 724 & V4718 < 999999999999)) GRUPO_REND=2.
IF  ((V4718 = 999999999999)) GRUPO_REND=9.
EXECUTE.

*Define Variable Properties.
*GRUPO_REND.
VARIABLE LEVEL GRUPO_REND  (NOMINAL).
VALUE LABELS  GRUPO_REND 
     1  'Rendimento <= 1 SM'
     2  'Rendimento > 1 SM'
     9  'Ignorado'  .
EXECUTE.
...
* Custom Tables.
CTABLES
  /VLABELS VARIABLES=POTP V0302 GRUPO_REND V4718 Idade DISPLAY=DEFAULT
  /TABLE Idade BY POTP > V0302 > GRUPO_REND > V4718 [MEAN]
  /CATEGORIES VARIABLES=POTP V0302 GRUPO_REND ORDER=A KEY=VALUE EMPTY=INCLUDE
  TOTAL=YES POSITION=AFTER
  /CATEGORIES VARIABLES=Idade ORDER=A KEY=VALUE EMPTY=EXCLUDE TOTAL=YES
  POSITION=AFTER.
