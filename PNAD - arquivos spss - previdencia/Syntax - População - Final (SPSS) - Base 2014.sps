...
GET
  FILE='F:\Modelo de Projeção - Atualização\PNAD 2014\Base - PNAD - 2014.sav'.
...
*PDTP.
VARIABLE LEVEL PDTP  (NOMINAL).
*POTP.
VARIABLE LEVEL POTP  (NOMINAL).
*POUR.
VARIABLE LEVEL POUR  (NOMINAL).
EXECUTE.
...
* Custom Tables.
CTABLES
  /VLABELS VARIABLES=V0302 POUR Idade DISPLAY=DEFAULT
  /TABLE Idade [COUNT F40.0] BY V0302 > POUR
  /CATEGORIES VARIABLES=V0302 POUR ORDER=A KEY=VALUE EMPTY=INCLUDE TOTAL=YES
  POSITION=AFTER
